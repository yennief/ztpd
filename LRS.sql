...
where sdo_relate(sr.geom, sdo_geom.sdo_buffer(mc.geom, 10, 1, 'unit=km'),'MASK=ANYINTERACT') = 'TRUE' and mc.city_name = 'Koszalin';

insert into A6_LRS
select sr.geom from STREETS_AND_RAILROADS sr where sr.id = 56;
...
values ('A6_LRS', 'Geom', mdsys.sdo_dim_array(MDSYS.SDO_DIM_ELEMENT('X', 12.603676, 26.369824, 1),MDSYS.SDO_DIM_ELEMENT('Y', 45.8464, 58.0213, 1),
...
-- 2 
-- A
select SDO_LRS.VALID_MEASURE(GEOM, 1000) VALID_1000, SDO_LRS.VALID_MEASURE(GEOM, 500) VALID_500
from A6_LRS;
...
-- F
select sdo_geom.sdo_length(sdo_lrs.offset_geom_segment(A6.geom, m.diminfo, 50, 200, 50, 'unit=m arc_tolerance=1'), 1, 'unit=km') cost
